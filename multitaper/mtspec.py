...
class MTSpec:
    ...
    def qiinv(self):
        ...
        *Calls*
        ...
        qispec, ds, dds = utils.qiinv(
            self.spec, self.yk, self.wt, self.vn, self.lamb, self.nw
        )
    ...
    def __init__(self, x, ntap=0, ntimes=0, fact=1.0, dt=1.0):
        ...
        **Notes**
        ...
        if ntap > 0:
            ...
            spec, kopt = utils.sadapt(nfft, fx, nf, df, itap, ntimes, fact)
        ...
        err2 = spec / (1 - 2.0 / (9.0 * v) + 1.96 * np.sqrt(2.0 / (9.0 * v))) ** 3
        err[:, 0] = err1
        err[:, 1] = err2
...
def spectrogram(
    data, dt, twin, olap=0.5, nw=3.5, kspec=5, fmin=0.0, fmax=-1.0, iadapt=0
):
    """
    ...
    **Examples**
    ...
    for i in range(nspec):
        ...
        psd = MTSpec(x, nw, kspec, dt, iadapt=iadapt, vn=vn, lamb=theta)
