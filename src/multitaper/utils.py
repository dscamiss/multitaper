...
def dpss(npts, nw, kspec=None):
    ...
    **Calls**
    ...
    if npts < 1e5:
        v, lamb2 = signal.windows.dpss(
            npts, nw, Kmax=kspec, sym=True, norm=2, return_ratios=True
        )
        ...
        v2int = signal.windows.dpss(nint, nw, Kmax=kspec, sym=True, norm=2)
...
def dpss2(npts, nw, nev=None):
    ...
    **Calls**
    ...
    if nodd > 0:
        ...
        if nx == 0:
            fv1[lh - 1] = (
                com * (hn - float(lh - 1)) ** 2 - float(lh * (npts - lh)) / 2.0
            )

        eigval, v2 = linalg.eigh_tridiagonal(
            fv1, fv2[1:lh], select="i", select_range=(lh - nodd, lh - 1)
        )
...
def qiinv(spec, yk, wt, vn, lamb, nw):
    ...
    **Calls**
    ...
    for i in range(nfft):
        ...
        # ---------------------------------------------
        cte_out = optim.nnls(np.real(h1), np.real(Cjk[:, 0]))[0]
    ...
    **Calls**
    ...
    for i in range(nf):
        ...
        for j in range(nf):
            ...
            df_phase[i, j] = np.arctan2(np.imag(df_spec[i, j]), np.real(df_spec[i, j]))
    ...
    **References**
    ...
    for m in range(nf):
        ...
        for k0 in range(klim):
            ...
            sxy[m, 2] = (
                sxy[m, 2] + (np.real(z1) * np.real(z2) + np.imag(z1) * np.imag(z2)) * wk
            )
            sxy[m, 3] = (
                sxy[m, 3] + (np.real(z2) * np.imag(z1) - np.real(z1) * np.imag(z2)) * wk
            )
    ...
    for iter in range(ntimes):
        for ipsd in range(2):
            ...
            for j in range(nf):
                ...
                ak = float(kopt[j]) / float(2 * ispan)
                phi = (
                    720.0 * ak**5 * (1.0 - 1.286 * ak + 0.476 * ak**3 - 0.0909 * ak**5)
                )
                sigR = np.sqrt(phi / float(kopt[j]) ** 5) / df**2
